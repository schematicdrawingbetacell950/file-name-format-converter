...
> ※ 브라우저 보안 제한으로 개별 파일이 아닌 **폴더 단위**로만 변환할 수 있습니다. 파일이 들어있는 폴더를 통째로 드롭해주세요.
...
>
> 또한 드래그앤드롭으로 얻은 `FileSystemFileHandle`은 `move()` 호출이 불가능합니다.
> 이는 macOS와 Windows 모두 동일한 브라우저 보안 제한입니다.
>
> **폴더째로 드롭하면** 디렉토리 핸들의 `values()`를 순회하면서
> 파일시스템의 실제 NFD 이름을 읽으므로 정상 감지되며, `move()`도 정상 동작합니다.
...
<details>
<summary><strong>Google Drive 자동 변환 (Apps Script)</strong></summary>

&nbsp;

> Google Drive 공유 폴더에 Mac 사용자가 파일을 올리면
> 자동으로 파일명을 변환해주는 스크립트를 설정할 수 있습니다.
>
> 한 번 설정하면 이후에는 양쪽 사용자 모두 아무 작업이 필요 없습니다.
>
> 👉 [설정 가이드 보기](./google-apps-script/)
...
<details>
<summary><strong>Gmail 첨부파일은?</strong></summary>

&nbsp;

> Gmail은 첨부파일의 파일명을 자동으로 변환하지 않습니다.
>
> Mac에서 파일을 메일로 보내기 전에 이 도구로 미리 파일명을 변환하세요.
>
> Gmail 자체의 자동 변환은 현재 기술적으로 불가능합니다.
